...
class AddDocumentsViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate {
    ...
    func savePhotoToCoreData(child: String, date: Date, image: UIImage, title: String, userName: String) {
        guard let appDelegate = UIApplication.shared.delegate as? AppDelegate else { return }
        let context = appDelegate.persistentContainer.viewContext
        ...
        document.setValue(user, forKey: "userName")
    ...
    func updateDocument(at indexPath: IndexPath, newTitle: String) {
        ...
        // Fetch the document based on the user, childName, and the existing title at the given index
        let fetchRequest = NSFetchRequest<NSManagedObject>(entityName: "Documents")
        ...
        do {
            ...
            if let document = documents.first {
                ...
                // Save the context after modifying
                try context.save()
                
                // Update the local photoNames array and save it to UserDefaults
                ...
                print("Document updated with new title: \(newTitle)")
            }
        } catch {
    ...
    func showNameInputAlert(for image: UIImage) {
        ...
        let saveAction = UIAlertAction(title: "Save", style: .default) { [weak self, weak alertController] _ in
            ...
            self?.savePhotoToCoreData(child: self?.childName ?? "", date: Date(), image: image, title: photoName, userName: self?.user ?? "")
        }
